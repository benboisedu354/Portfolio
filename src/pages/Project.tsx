  ...
  return (
    <div className="pt-20 min-h-screen bg-gradient-to-b from-darker to-dark">
      <div className="max-w-6xl mx-auto px-4 pt-10">
        <Link
          to="/projects"
          className=" text-md text-indigo-400 hover:text-indigo-500 transition-colors "
        >
      ...
      <div className="max-w-5xl mx-auto px-4 py-20">
        ...
        <div className="flex flex-wrap gap-4 mb-6 items-center">
          {project?.tags.map((tag, i) => (
            <span
              key={i}
              className="px-3 py-1 bg-indigo-500/10 text-indigo-400 rounded-full text-sm hover:bg-indigo-500/20 transition-colors cursor-pointer "
            >
              {/* Redirect to projects page with tag filter */}
              <Link
                to={`/projects?tag=${tag}`}
                className="no-underline text-indigo-400 hover:text-indigo-500 transition-colors"
              >
                {tag}
              </Link>
            </span>
